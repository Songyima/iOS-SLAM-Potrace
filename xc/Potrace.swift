...
final class Potrace {
    ...
    /*
     turdsize:removing all curves whose enclosed area is below the given threshold.
     
     turnpolicy:how to resolve ambiguities during decomposition of bitmaps into paths.
     alphamax:a threshold for the detection of corners.from 0.0 (polygon) to 1.3334 (no corners).It controls the smoothness.
     optcurve:whether Potrace will at- tempt to “simplify” the ﬁnal curve by reducing the number of Bezier curve segments.Opticurve=1 turns on optimization, and opticurve=0 turns it off
     opttolerance:The opttolerance parameter deﬁnes the amount of error allowed in this sim- pliﬁcation.Larger values tend to decrease the number of segments, at the expense of less accuracy.
     */
...
//        print("bmToPath", endTime-startTime)
